...
Với dữ liệu đầu vào là câu dạng ngôn ngữ tự nhiên, chúng tôi xây dựng 2 chương trình con để xác định cấu trúc phủ định trong câu:
\begin{itemize}
\item Meta-NegEx: dùng tùy chọn \myquote{-negex} để lấy được kết quả phân tích của MetaMap có chứa từ phủ định và phạm vi phủ định.

Ví dụ như khi áp dụng Meta-NegEx với câu \myquote{In this trial of apparently healthy persons without hyperlipidemia but with elevated high-sensitivity C-reactive protein levels, rosuvastatin significantly reduced the incidence of major cardiovascular events}, kết quả lưu lại dưới dạng JSON như Hình~\ref{fig:Meta-NegEx}. Trong đó, thuộc tính \myquote{id} là mã số câu, thuộc tính \myquote{negations} là mảng chứa các cấu trúc phủ định trong câu, bao gồm từ phủ định chứa trong thuộc tính \myquote{negex} và mảng các từ bị phủ định chứa trong thuộc tính \myquote{effectedWords}.
