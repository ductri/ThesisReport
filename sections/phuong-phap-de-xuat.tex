...
Khối \textbf{Thu thập và đánh nhãn dữ liệu:} Nhóm tiến hành thu thập dữ liệu từ nguồn web, lưu vào hệ cơ sở dữ liệu, sau đó tiến hành đánh nhãn. Chi tiết được trình bày tại Mục \ref{sec:thu-thap-va-danh-gia-du-lieu}\\
Khối \textbf{Tiền xử lý:} Dữ liệu thu thập được có thể gặp lỗi và có định dạng không đúng, cần được xử lý trước khi trích xuất đặc trưng. 
...
\end{itemize}
Khối \textbf{Rút trích đặc trưng:} Có 4 đặc trưng được sử dụng: N-gram, change phrase, sự phủ định và SO-CAL được trình bày trong các Mục từ \ref{sec:ngram} đến \ref{sec:socal}. 
Khối \textbf{Huấn luyện:} Thực hiện việc huấn luyện với giải thuật học máy SVM để tạo ra bộ phân loại.\\
Khối \textbf{Phân loại:} Sử dụng bộ phân loại từ khối Huấn luyện, với mỗi dữ liệu đầu vào, bộ phân loại sẽ phân loại câu thuộc 1 trong 3 lớp: \tichcuc,\tieucuc, hoặc \trungtinh\\
Khối \textbf{Đánh giá:} Sử dụng khối phân loại ở trên, áp dụng đầu vào là tập dữ liệu kiểm tra để đánh giá hiệu quả của bộ phân loại.
\begin{figure}[H]
\centering
